...
class InferenceManager extends ChangeNotifier {
  ...
  final Map<int, DateTime> _roiCooldownByTrack = {};

  InferenceManager({required AppSettings settings, required LocalDetectionService localService, required CloudDetectionService cloudService, required NetworkMonitor networkMonitor})
      : _settings = settings,
  ...
  Future<void> processFrame(Uint8List frameData, {int width = 0, int height = 0}) async {
    final minFrameIntervalMs = (1000 / _settings.targetFps).round();
    if (DateTime.now().difference(_lastFrameProcessedAt).inMilliseconds < minFrameIntervalMs) return;
    final accepted = _frameBuffer.push(frameData, metadata: {'width': width, 'height': height});
    if (!accepted || _isProcessing) return;
    _isProcessing = true;
    try {
      while (_frameBuffer.size > 0) {
  ...
  Future<void> _runInference(Uint8List frameData, {int width = 0, int height = 0}) async {
    ...
    if (mode == InferenceMode.localOnly || mode == InferenceMode.hybrid) {
      ...
      try { detections = await _localService.detect(frameData, width: width, height: height); } catch (e) { debugPrint('[InferenceManager] Local detection error: $e'); }
    }
    if (mode == InferenceMode.cloudOnly) {
      _state = InferenceState.cloudInference;
      try { detections = await _cloudService.detect(frameData, width: width, height: height); } catch (e) { debugPrint('[InferenceManager] Cloud detection error: $e'); }
    }
    final tracked = _tracker.update(detections);
    _currentDetections = _mergeWithTracked(detections, tracked);
    if (mode == InferenceMode.hybrid && _settings.cloudUploadConsentGranted) {
      for (final det in _currentDetections) {
        final shouldSendToCloud = det.confidence < _settings.minConfidence || _settings.detailedAnalysisWhitelist.any((w) => det.label.toLowerCase().contains(w.toLowerCase()));
        if (shouldSendToCloud && _canRunCloudRoi(det.trackId)) { _triggerCloudAnalysis(det, frameData); }
      }
    }
  ...
  bool _canRunCloudRoi(int trackId) {
    if (trackId == kNoTrackId) return true;
    final now = DateTime.now();
    final last = _roiCooldownByTrack[trackId];
    if (last != null && now.difference(last).inSeconds < 2) return false;
    _roiCooldownByTrack[trackId] = now;
    return true;
  }
    ...
    try {
      ...
      if (matchedTrack != null) {
        ...
        if (idx >= 0) {
          ...
          _currentDetections[idx] = _currentDetections[idx].copyWith(cloudMetadata: response);
          notifyListeners();
        }
      }
    } catch (e) {
